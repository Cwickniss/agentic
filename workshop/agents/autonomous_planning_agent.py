...
class AutonomousPlanningAgent(Agent):
    ...
    def scan_the_internet_for_bargains(self) -> Dict:
        """
    ...
    def estimate_true_value(self, description: str) -> Dict:
        """
        Run the tool to estimate true value
        """
        self.log(f"Autonomous Planning agent is estimating value")
        estimate1 = self.frontier.price(description)
        estimate2 = self.specialist.price(description)
    ...
    def get_tools(self):
        ...
        """
        return [{"type": "function", "function": self.scan_function},
    ...
    def handle_tool_call(self, message):
        ...
        """
        mapping = {"scan_the_internet_for_bargains": self.scan_the_internet_for_bargains, "estimate_true_value": self.estimate_true_value, "notify_user_of_deal": self.notify_user_of_deal}
        results = []
    ...
    user_message += "You must only notify the user about one deal, and be sure to pick the most compelling deal, where the deal price is much lower than the estimated true value. Only notify the user once. Then just respond OK to indicate success."
